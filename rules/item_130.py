"""
Rule Item 130: Remote Location (4km from nearest pharmacy)
"""
...
class Item130Rule(BaseRule):
    """
    A pharmacy is eligible if it is at least 4km from the nearest existing pharmacy.
    Distance is measured straight-line from ground-level public access door midpoints.
    """

    @property
    def rule_name(self) -> str:
        return "Remote Location (4km)"
    ...
    def check_eligibility(self, property_data: Dict) -> Tuple[bool, Optional[str]]:
        """
        Check if property is at least 4km from nearest pharmacy.
        ...
        if distance_km >= threshold:
            evidence = self.format_evidence(
                rule="4km from nearest pharmacy",
